// Built-in themes share the same API, authentication and node actions.
// Hex is a native VibeMonitor adaptation inspired by 8bitNull/monitor-theme-hex.
window.VibeThemes = (() => {
  const catalog = Object.freeze({ default: '默认主题', hex: 'Hex' });
  const normalize = value => Object.hasOwn(catalog, value) ? value : 'default';
  ...
  function renderHexNode(node) {
    ...
    return `<article class="node-card hex-card ${node.online ? '' : 'offline'}">
      ...
      <footer class="hex-card-footer"><span>${escapeHtml(node.group || '未分组')}</span><strong>${bill.price}</strong></footer>
    </article>`;
  }
  ...
  return Object.freeze({ catalog, normalize, renderHexNode, renderOverview });
})();
