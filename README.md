...
主控只信任本机回环代理的 HTTPS 标记，用于安全 Cookie 和动态安装链接。若使用其他本机反向代理，请覆盖 `X-Forwarded-For` 和 `X-Real-IP`，不要转发客户端自带的值；这两个头仅用于记录和展示探针 IP。探针 RPC 上报密钥必须放在 `Authorization: Bearer` 或 `X-Token` 请求头中；RPC 请求只要包含 `?token=` 就会被拒绝，仍使用查询参数密钥的旧探针需要升级。动态安装链接 `/install.sh?token=...` 仍含节点密钥，不要公开分享或保留到公开访问日志。
...
流量统计默认排除回环、常见容器/隧道接口，并通过 sysfs 排除网桥、bond、VLAN、macvlan 等可能与底层网卡重复计数的接口；bond 的物理成员网卡会被计入。复杂网络建议明确指定出口网卡，例如 `vibemonitor agent --server https://monitor.example.com --token YOUR_TOKEN --interfaces eth0`；多个接口使用逗号分隔，勿同时选择同一流量经过的物理接口与隧道接口。systemd 安装可在探针服务的 drop-in 中设置 `Environment="VIBEMONITOR_INTERFACES=eth0"`，然后重新加载并重启探针。接口不存在或读取失败时不提交零流量报告。新版探针上报系统重启标识：重启或计数器下降时重建对应基线，统计接口集合变化时继续累计未变化网卡的增量，保留已累计用量，不回算历史误差。部署时先更新主控，再更新探针；旧版探针仍可上报，但不具备重启标识和新版网卡筛选。

未填写每月流量重置日时默认使用 1 日，所有节点按月统计上传与下载之和。已有节点首次启用此默认值时从最后一次上报的计数器继续统计，不补算之前未记录的流量。新版探针会上报每张选定网卡的计数器，单张网卡重置时分别计算差分；旧版探针仍可上报合计值，但不具备此修正。上报间隔跨过账期边界时，主控按采样时间比例估算新周期用量；累计计数器无法还原间隔内每一刻的精确流量。

节点管理的编辑窗口可手动设置当前周期已用流量，留空保持原值，填写 `0` 可清零。校准时会记录时间，之后仅累加校准时间之后的探针流量。探针报告的 `updated_at` 是流量计数器采样时间；主控用可信的采样时间确定账期和分摊跨边界增量，缺失或明显偏离接收时间时回退到主控接收时间。
